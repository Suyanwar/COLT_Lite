	...
	<div class="modal-body">
		<div id="content">
			<ul id="tabs" class="nav nav-tabs" data-tabs="tabs">
				...
				<li><a href="#TrendingPost" data-toggle="tab">Trending Tweet</a></li>
			</ul>
		</div>
...
$('#primetime_TrendingPost').highcharts({
    ...
    series: [{
        ...
        data:[
 			<?php
				for($i=0; $i < 24; $i++)
				$sph[$i] = $this->report_twitter->primetime($account, 'SumPost', array($kurangsatu, '%H', $i));
				ksort($sph);
				$k = 0;
				foreach ($sph as $key => $value) {
					echo $value;
        ...
        data:[
 			<?php
				for($i=0; $i < 24; $i++)
				$sph[$i] = $this->report_twitter->primetime($account, 'SumPost', array($kurangdua, '%H', $i));
				ksort($sph);
				$k = 0;
				foreach ($sph as $key => $value) {
					echo $value;
...
$(document).ready(function(e){
	...
	$('#dateRangePicker').on('apply.daterangepicker', function(ev, picker) {
		report_load('twitter', 'primetime', <?php echo $account;?>, picker.startDate.format('DD-MMM-YYYY') + " ~ " + picker.endDate.format('DD-MMM-YYYY'));
	});
});
</script>