            ...
            <?php
                if($data){
                    ...
                    foreach($data as $list){
                        ...
                        $temp = $this->report_facebook->activities($list->account_id, 'TotalFeedback', $date);
                        foreach ($temp as $t) {
                            $s += $t->total;
                        }
                                ...
                                for ($h=0; $h < count($row['name']); $h++) { 
                                    ...
                                    for($i=$start; $i <= $end; $i++){
                                        for($m=1; $m <= count($arr); $m++){
                                            $current = ($i.$m == $cur_month) ? 0 : $this->report_facebook->growth($row['id'][$h], 'MonthlyFans', $i.'-'.(($m > 9) ? $m : '0'.$m));
                                            if($x){
                                                array_push($c, $current ? $current - $last : 0);
                                                array_push($d, $current ? number_format($current - $last) : 0);
                                            }
                                            else{ 
                                                array_push($c, $current - $this->report_facebook->growth($row['id'][$h], 'DailyPrevious', $i.'-01-01'));
                                                array_push($d, number_format($current - $this->report_facebook->growth($row['id'][$h], 'DailyPrevious', $i.'-01-01')));
                                            }
                                            $last = $current;
                                            $x=1;
                                        }
...
$(document).ready(function(e){
    ...
    $('#dateRangePicker').on('apply.daterangepicker', function(ev, picker) {
        report_load('facebook', 'benchmark', <?php echo $account->account_id;?>, picker.startDate.format('DD-MMM-YYYY') + " ~ " + picker.endDate.format('DD-MMM-YYYY'));
    });
});
