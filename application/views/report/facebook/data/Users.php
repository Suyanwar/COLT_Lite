			...
			<div class="tab-pane active" id="TopSpeaker">
				<?php
					...
					foreach($top_speakers as $list){
						echo '<li><table><tr><td><a href="https://www.facebook.com/'.($list->u ? $list->u : $list->s).'" target="_blank"><img src="'.$list->p.'"></a></td><td>Amplification: <b>'.number_format($list->i).'</b><br>Activity: <b>'.number_format($list->t).'</b><br>Friends: <b>'.(($list->f > 1) ? number_format($list->f) : '').'</b><br>Loc: <b>'.$list->l.'</b></td></tr><tr><td colspan="2"><div><a href="https://www.facebook.com/'.($list->u ? $list->u : $list->s).'" target="_blank">'.$list->n.'</a></div></td></tr></table></li>';
					}
					echo '</ul>';
			...
			<div class="tab-pane" id="MostActive">
				<?php
					...
					foreach($most_active as $list){
						echo '<li><table><tr><td><a href="https://www.facebook.com/'.($list->u ? $list->u : $list->s).'" target="_blank"><img src="'.$list->p.'"></a></td><td>Activity: <b>'.number_format($list->t).'</b><!--br>Friends: <b>'.(($list->f > 1) ? number_format($list->f) : '').'</b--><br>Loc: <b>'.$list->l.'</b></td></tr><tr><td colspan="2"><div><a href="https://www.facebook.com/'.($list->u ? $list->u : $list->s).'" target="_blank">'.$list->n.'</a></div></td></tr></table></li>';
					}
					echo '</ul>';
					echo '<div class="clear"></div>';
				}else echo '<p align="center" style="padding-top:25px">No data.</p>';
...
$(document).ready(function(e){
	...
	$('#dateRangePicker').on('apply.daterangepicker', function(ev, picker) {
		report_load('facebook', 'users', <?php echo $account;?>, picker.startDate.format('DD-MMM-YYYY') + " ~ " + picker.endDate.format('DD-MMM-YYYY'));
	});
});
