...
if ($highlight) {
	foreach($highlight as $list){
		...
		if(isset($er)){
			$er_old = $er;
			$er = ($this->report_facebook->users($account->account_id, 'MostActiveCount', $date) / ($list['fans'] ? $list['fans'] : 1)) * 100;
			if($er > $er_old){
		...
		else $er = ($this->report_facebook->users($account->account_id, 'MostActiveCount', $date) / ($list['fans'] ? $list['fans'] : 1)) * 100;
...
<?php
	if($AllAccount){ 
		foreach($AllAccount as $list){
			...
			echo '<td>Total Feedback: <b>'.number_format($list->total).'</td>';
			echo '</tr>';
...
$(document).ready(function(e){
	...
	$('#dateRangePicker').on('apply.daterangepicker', function(ev, picker) {
		report_load('facebook', 'highlight', <?php echo $account->account_id;?>, picker.startDate.format('DD-MMM-YYYY') + " ~ " + picker.endDate.format('DD-MMM-YYYY'));
	});
});
