						...
						<?php
							if($data){
								...
								foreach($data as $list){
									...
									echo '</tr>';
									$engagement_ratio = ($this->report_facebook->users($list->account_id, 'MostActiveCount', $fdate) / ($list->fans ? $list->fans : 1)) * 100;
...
<?php
    if($data){
        ...
        foreach($data as $list){
            ...
            $temp = $this->report_facebook->activities($list->account_id, 'TotalFeedback', $fdate);
...
$('#performance_VsCompetitor').highcharts({
	...
	series: [
		...
		for ($i=0; $i < count($com['id']); $i++) {
			...
			echo "data: [";
			for ($j=2; $j >=0; $j--) { 
				echo $this->report_facebook->performance($com['id'][$i], 'VsCompetitor', date('Y-m', strtotime("- $j month", $unix)));
				if ($j>0) echo ",";
			}
			echo "]}";
...
$(document).ready(function(e){
	...
	$('#dateRangePicker').on('apply.daterangepicker', function(ev, picker) {
		report_load('facebook', 'performance', <?php echo $account->account_id;?>, picker.startDate.format('DD-MMM-YYYY') + " ~ " + picker.endDate.format('DD-MMM-YYYY'));
	});
});
</script>