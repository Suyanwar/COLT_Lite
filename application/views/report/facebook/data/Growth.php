			...
			<div class="tab-pane" id="CompareMonthlyGrowth">
				...
				<div id="growth1_MonthlyGrowthTable">
					<table width="100%" cellspacing="0" cellpadding="0" border="0" class="table">
						...
						<tbody id="Customdata" class="center">
						</tbody>
					</table>
				</div>
			</div>
		...
		data: [
			<?php
				...
				foreach($dailyGrowth as $list){
					...
					else echo $list->count - $this->report_facebook->growth($account['account_id'], 'DailyPrevious', date('Y-m-d', $unix[0]));
					$last = $list->count;
					$i=1;
				}
			?>
		...
		data: [
			<?php
				...
				for($i=$start; $i <= $end; $i++){
					for($m=1; $m <= count($arr); $m++){
						echo $x ? ',' : '';
						echo ($i.$m == $cur_month) ? 0 : $this->report_facebook->growth($account['account_id'], 'MonthlyFans', $i.'-'.(($m > 9) ? $m : '0'.$m));
						$x=1;
					}
				}
			?>
	...
	series: [
			<?php
				...
				for($i=$start-1; $i <= $start; $i++){
					...
					echo "data: ";
					for($m=1; $m <= count($arr); $m++){
						$current = ($i.$m == $cur_month) ? 0 : $this->report_facebook->growth($account['account_id'], 'MonthlyFans', $i.'-'.(($m > 9) ? $m : '0'.$m));
						array_push($temp['total'], intval($current));
						if($x){
							array_push($temp['growth'], $current ? $current - $last : 0);
						}
						else{
							array_push($temp['growth'], $current - $this->report_facebook->growth($account['account_id'], 'DailyPrevious', $i.'-01-01'));
						}
						$last = $current;
						$x=1;
					}
...
if($data):
    ...
    foreach($data as $list):
		...
		for($i=$start; $i <= $end; $i++){
			for($m=1; $m <= count($arr); $m++){
				$current = ($i.$m == $cur_month) ? 0 : $this->report_facebook->growth($list->account_id, 'MonthlyFans', $i.'-'.(($m > 9) ? $m : '0'.$m));
				array_push($temp['total'], intval($current));
				if($x){
					array_push($temp['growth'], $current ? $current - $last : 0);
					
				}
				else array_push($temp['growth'], $current - $this->report_facebook->growth($list->account_id, 'DailyPrevious', $i.'-01-01')); 
				$last = $current;
				$x=1;
			}
		}
...
$(document).ready(function(e){
	...
	$('#dateRangePicker').on('apply.daterangepicker', function(ev, picker) {
		report_load('facebook', 'growth', <?php echo $account['account_id'];?>, picker.startDate.format('DD-MMM-YYYY') + " ~ " + picker.endDate.format('DD-MMM-YYYY'));
	});
});
</script>